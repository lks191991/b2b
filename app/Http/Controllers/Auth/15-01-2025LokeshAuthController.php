    ...
    public function dashboard()
    {
       ...
       $expiringDate = Carbon::now()->addMonths(1)->format('Y-m-d');
        ...
        if(Auth::check()){
                ...
                $startDateOfYear = Carbon::createFromDate($currentYear, 1, 1);
                $endDateOfYear = Carbon::createFromDate($currentYear, 12, 31);
				$dateOneYearAgo = $currentDate->copy()->subYear();
                

    $vouchersCurrentDate = DB::table('voucher_activity as va')
	->select(
        DB::raw('COUNT(DISTINCT va.voucher_id) AS totalVouchers'),
        DB::raw('SUM(CASE WHEN va.adult IS NOT NULL THEN va.adult ELSE 0 END) AS totalAdult'),
        DB::raw('SUM(CASE WHEN va.child IS NOT NULL THEN va.child ELSE 0 END) AS totalChild'),
		DB::raw('SUM(CASE WHEN va.actual_total_cost IS NOT NULL THEN va.original_tkt_rate ELSE 0 END) AS totalSells'),
        DB::raw('COUNT(*) AS totalActivity'),
        ...
        ->whereDate('v.booking_date',  $currentDate)
        ->where('v.zone', '!=', '');
		$vouchersCurrentDate = $vouchersCurrentDate->first();	

        $vouchersMonth = DB::table('voucher_activity as va')
	->select(
        DB::raw('COUNT(DISTINCT va.voucher_id) AS totalVouchers'),
        DB::raw('SUM(CASE WHEN va.adult IS NOT NULL THEN va.adult ELSE 0 END) AS totalAdult'),
        DB::raw('SUM(CASE WHEN va.child IS NOT NULL THEN va.child ELSE 0 END) AS totalChild'),
        ...
        ->leftJoin('voucher_hotels as vh', 'v.id', '=', 'vh.voucher_id')
        ->where('v.status_main', 5)
        ->whereDate('v.booking_date', '>=', $currentMonthStartDate)
        ->whereDate('v.booking_date', '<=', $currentDate)
        ->where('v.zone', '!=', '');
		$vouchersMonth = $vouchersMonth->first();	

        $vouchersYear = DB::table('voucher_activity as va')
        ->select(
            DB::raw('COUNT(DISTINCT va.voucher_id) AS totalVouchers'),
            DB::raw('SUM(CASE WHEN va.adult IS NOT NULL THEN va.adult ELSE 0 END) AS totalAdult'),
            DB::raw('SUM(CASE WHEN va.child IS NOT NULL THEN va.child ELSE 0 END) AS totalChild'),
            ...
            ->leftJoin('voucher_hotels as vh', 'v.id', '=', 'vh.voucher_id')
            ->where('v.status_main', 5)
            ->whereDate('v.booking_date', '>=', $startDateOfYear)
            ->whereDate('v.booking_date', '<=', $endDateOfYear)
            ->where('v.zone', '!=', '');
            $vouchersYear = $vouchersYear->first();	
